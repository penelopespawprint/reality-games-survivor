...
export default function ProfileSetup() {
  ...
  // The database trigger creates a profile with display_name = email prefix
  // So we check if display_name is the auto-generated one (matches email prefix)
  const emailPrefix = user?.email ? user.email.split('@')[0] : '';
  const isAutoGeneratedName = profile?.display_name === emailPrefix;

  // If user has a custom display name (not auto-generated), they've completed setup
  if (profile?.display_name && !isAutoGeneratedName) {
