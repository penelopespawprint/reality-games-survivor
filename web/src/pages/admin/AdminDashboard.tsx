...
export function AdminDashboard() {
  ...
  const { data: stats } = useQuery({
    ...
    queryFn: async () => {
      const [users, leagues, castaways, episodes, scoringRules] = await Promise.all([
        ...
        activeSeason?.id
          ? supabase.from('castaways').select('id', { count: 'exact', head: true }).eq('season_id', activeSeason.id)
          : { count: 0 },
        activeSeason?.id
          ? supabase.from('episodes').select('id', { count: 'exact', head: true }).eq('season_id', activeSeason.id)
          : { count: 0 },
        activeSeason?.id
          ? supabase.from('scoring_rules').select('id', { count: 'exact', head: true }).eq('season_id', activeSeason.id)
          : { count: 0 },
      ]);
  ...
  const adminLinks = [
    ...
    },
    {
      ...
      description: 'Create and edit episode schedule',
      href: '/admin/episodes',
      icon: (
        <svg className="w-8 h-8" fill="none" viewBox="0 0 24 24" stroke="currentColor">
      ...
      <div className="min-h-screen bg-gradient-to-b from-cream-100 to-cream-200">
        ...
        <main className="max-w-4xl mx-auto px-4 py-16 text-center">
          <div className="bg-white rounded-2xl shadow-elevated p-12">
            ...
            <p className="text-neutral-500 mb-8">You don't have permission to access the admin dashboard.</p>
      ...
      <main className="max-w-6xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
        ...
        <div className="flex items-center justify-between mb-8 animate-fade-in">
          <div>
            ...
            <p className="text-neutral-500">
              {activeSeason ? `Season ${activeSeason.number}: ${activeSeason.name}` : 'No active season'}
            </p>
          </div>
          <Link to="/dashboard" className="btn bg-white text-neutral-700 shadow-card hover:shadow-card-hover">
            Back to App
          </Link>
        </div>
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6 animate-slide-up" style={{ animationDelay: '0.1s' }}>
          {adminLinks.map((link) => (
            ...
            >
              <div className={`w-14 h-14 ${link.color} rounded-xl flex items-center justify-center text-white mb-4 group-hover:scale-110 transition-transform`}>
                {link.icon}
              </div>
