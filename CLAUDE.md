...
- ❌ Never write direct Supabase mutations in frontend for sensitive ops (use API)
- ❌ Never store passwords in plaintext (use bcrypt on backend)
- ⚠️ Always use `apiWithAuth()` for authenticated API calls
- ⚠️ Always import types from `@/types` (never define inline)
- ⚠️ Always use shared hooks from `@/lib/hooks` (never inline queries)
- 💡 Prefer extracting components when files exceed 400 lines
- 💡 Prefer PST/PDT timezone handling with Luxon
...
## Common Pitfalls
1. **Problem:** Query key mismatch between hooks → **Solution:** Always use hooks from `@/lib/hooks`
2. **Problem:** Timezone bugs with episode times → **Solution:** Use Luxon, all times in PST
3. **Problem:** Admin routes accessible without auth → **Solution:** All admin ops go through `/api/admin/*` with role check
4. **Problem:** Roster shows dropped castaways → **Solution:** Filter `.is('dropped_at', null)`
