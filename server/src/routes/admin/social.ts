...
async function bufferFetch(endpoint: string, options?: RequestInit): Promise<any> {
  ...
  if (!response.ok) {
    ...
    throw new Error((errorData as { message?: string }).message || `Buffer API error: ${response.status}`);
  }
...
router.get('/profiles/:profileId/pending', async (req, res: Response) => {
  try {
    const { profileId } = req.params;
    const { page = 1, count = 20 } = req.query;

    const result = await bufferFetch(
      `/profiles/${profileId}/updates/pending.json?page=${page}&count=${count}`
    );
...
router.get('/profiles/:profileId/sent', async (req, res: Response) => {
  try {
    const { profileId } = req.params;
    const { page = 1, count = 20 } = req.query;
...
router.delete('/posts/:postId', async (req, res: Response) => {
  try {
    const { postId } = req.params;
...
router.post('/posts/:postId/share', async (req, res: Response) => {
  try {
    const { postId } = req.params;
...
router.post('/posts/:postId/move-to-top', async (req, res: Response) => {
  try {
    const { postId } = req.params;
