...
router.patch('/:id', async (req: AuthenticatedRequest, res: Response) => {
  try {
    const { id } = req.params;
    const { status, title, description, affectedSystems, usersAffected, workaround } = 
    ...
    if (affectedSystems !== undefined) updates.affected_systems = affectedSystems;
    if (usersAffected !== undefined) updates.users_affected = usersAffected;
    if (workaround !== undefined) updates.workaround = workaround;
 ...
 */
router.get('/active/count', async (req: AuthenticatedRequest, res: Response) => {
  try {
