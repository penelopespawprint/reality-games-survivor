...
router.get('/executive', async (req: AuthenticatedRequest, res: Response) => {
  try {
    // Get user funnel
    ...
    const healthScore = Math.round(
      (retentionRate * 0.5) + (activationRate * 0.3) + (Math.min(growthRate * 10, 20))
    );
    ...
    const topLeaguesWithStats = await Promise.all(
...
router.get('/engagement', async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    for (const user of users || []) {
      ...
      if (segment && segments[segment as keyof typeof segments] !== undefined) {
...
router.get('/history', async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    const groupedData = (data || []).reduce((acc: Record<string, Array<{ date: string; value: number }>>, item) => {
