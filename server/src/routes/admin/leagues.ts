...
router.get('/', async (req: AuthenticatedRequest, res: Response) => {
  try {
    ...
    const enhancedLeagues = await Promise.all(
...
router.post('/:id/transfer', async (req: AuthenticatedRequest, res: Response) => {
  try {
    const { id } = req.params;
...
router.post('/:id/message', async (req: AuthenticatedRequest, res: Response) => {
  try {
    const { id } = req.params;
