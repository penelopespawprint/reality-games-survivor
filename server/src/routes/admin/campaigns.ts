...
router.get('/email', async (req, res: Response) => {
  try {
...
// Get single campaign
router.get('/email/:id', async (req, res: Response) => {
  try {
    const { id } = req.params;
...
router.post('/email', async (req: AuthenticatedRequest, res: Response) => {
  try {
...
router.put('/email/:id', async (req: AuthenticatedRequest, res: Response) => {
  try {
    const { id } = req.params;
    const userId = req.user?.id;
    const parsed = emailCampaignSchema.safeParse(req.body);
...
// Send campaign now
router.post('/email/:id/send', async (req: AuthenticatedRequest, res: Response) => {
  try {
    const { id } = req.params;
...
// Archive campaign
router.post('/email/:id/archive', async (req: AuthenticatedRequest, res: Response) => {
  try {
    const { id } = req.params;
    const userId = req.user?.id;
...
// Unarchive campaign
router.post('/email/:id/unarchive', async (req: AuthenticatedRequest, res: Response) => {
  try {
    const { id } = req.params;
...
router.delete('/email/:id', async (req, res: Response) => {
  try {
    const { id } = req.params;
...
router.get('/email/recipients/:segment', async (req, res: Response) => {
  try {
...
// List SMS campaigns
router.get('/sms', async (req, res: Response) => {
  try {
    const { status, limit = 50, offset = 0 } = req.query;
...
// Create SMS campaign
router.post('/sms', async (req: AuthenticatedRequest, res: Response) => {
  try {
    const userId = req.user?.id;
...
router.post('/sms/:id/send', async (req: AuthenticatedRequest, res: Response) => {
  try {
    const { id } = req.params;
...
router.post('/sms/:id/archive', async (req: AuthenticatedRequest, res: Response) => {
  try {
    const { id } = req.params;
    const userId = req.user?.id;
...
router.get('/sms/recipients/:segment', async (req, res: Response) => {
  try {
    const { segment } = req.params;
 ...
 */
async function getRecipientsBySegment(segment: string): Promise<Array<{
  email: string;
  display_name: string;
  user_id: string;
}>> {
 ...
 */
async function getSmsRecipientsBySegment(segment: string): Promise<Array<{
  phone: string;
  display_name: string;
  user_id: string;
}>> {
 ...
 */
async function sendEmailCampaignNow(campaignId: string): Promise<void> {
  try {
    ...
    // Get recipients
    let recipients: Array<{ email: string; display_name: string; user_id: string }>;
    
    if (campaign.recipient_segment === 'custom' && campaign.custom_recipients?.length) {
      recipients = campaign.custom_recipients.map((email: string) => ({
        email,
 ...
 */
async function sendSmsCampaignNow(campaignId: string): Promise<void> {
  try {
    const { data: campaign, error: fetchError } = await supabaseAdmin
      .from('sms_campaigns')
      .select('*')
    ...
    // Get recipients
    let recipients: Array<{ phone: string; display_name: string; user_id: string }>;
    
    if (campaign.recipient_segment === 'custom' && campaign.custom_recipients?.length) {
      recipients = campaign.custom_recipients.map((phone: string) => ({
        phone,
        display_name: '',
        user_id: '',
      }));
