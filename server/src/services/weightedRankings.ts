 ...
 */
function calculateConfidence(leagueCount: number): number {
 ...
 */
async function calculateGlobalMean(seasonId: string): Promise<number> {
  ...
  if (!data || data.length === 0) return 25; // Fallback default
 ...
 */
export async function getWeightedRankings(seasonId: string): Promise<WeightedRanking[]> {
 ...
 */
export function getConfidenceIndicator(leagueCount: number): string {
  if (leagueCount >= 5) return '✓✓✓';
  if (leagueCount >= 3) return '✓✓';
  if (leagueCount >= 2) return '✓';
  return '⚠️';
}
