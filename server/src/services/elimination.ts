 ...
 */
export async function eliminateCastaway(params: EliminateParams): Promise<EliminationResult> {
  ...
  for (const roster of rosters) {
    ...
    if (activeCastaways.length === 0) {
      ...
      if (user.notification_email !== false) notificationsSent.emailCount++;
      if (user.notification_sms && user.phone) notificationsSent.smsCount++;
 ...
 */
async function handleTorchSnuffed(
  user: { id: string; email: string; display_name: string; phone?: string; notification_email?: boolean; notification_sms?: boolean },
 ...
 */
async function handleEliminationAlert(
  user: { id: string; email: string; display_name: string; phone?: string; notification_email?: boolean; notification_sms?: boolean },
  league: { id: string; name: string },
  castawayName: string
): Promise<void> {
