...
async function emailQueueProcessor(): Promise<{ processed: number; failed: number }> {
  await new Promise((resolve) => setTimeout(resolve, 100)); // Simulate work
  return { processed: 5, failed: 0 };
}

async function lockPicks(): Promise<{ picksLocked: number }> {
...
async function demo() {
  ...
  history.forEach((exec, idx) => {
    ...
    console.log(
      `   ${status} ${exec.jobName.padEnd(25)} ${duration.padEnd(8)} ${exec.startTime.toISOString()}`
    );
  });
  console.log();
  ...
  if (emailStats.averageDurationMs) {
    ...
    const recentAvg =
      recent.reduce((sum, e) => sum + (e.durationMs || 0), 0) / recent.length;
