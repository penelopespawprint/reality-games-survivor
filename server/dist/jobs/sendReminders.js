...
export async function sendPickReminders() {
    ...
    for (const league of leagues) {
        ...
        for (const member of members || []) {
            if (pickedUserIds.has(member.user_id))
                continue;
            ...
            await EmailService.logNotification(member.user_id, 'email', `Pick reminder for Episode ${episode.number}`, `Reminder sent with ${hoursLeft} hours remaining.`);
            sent++;
        }
    }
...
export async function sendDraftReminders() {
    ...
    for (const league of leagues) {
        ...
        for (const member of members || []) {
            ...
            await EmailService.logNotification(member.user_id, 'email', `Draft reminder for ${league.name}`, `Reminder sent with ${daysLeft} days remaining.`);
            sent++;
        }
    }
