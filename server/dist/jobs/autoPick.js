...
export async function autoPick() {
    ...
    for (const league of leagues) {
        ...
        const missingUsers = members?.filter((m) => !pickedUserIds.has(m.user_id) && !m.eliminated_at) || [];
        for (const member of missingUsers) {
            ...
            if (!activeCastaways || activeCastaways.length === 0) {
                ...
                await handleEliminatedUser(member.user_id, league.id, league.name, episode.number, now);
                eliminatedUsers.push(member.user_id);
                console.log(`[AutoPick] User ${member.user_id} eliminated in league ${league.id} - torch snuffed`);
                continue;
            }
 ...
 */
async function handleEliminatedUser(userId, leagueId, leagueName, episodeNumber, eliminatedAt) {
    try {
...
//# sourceMappingURL=autoPick.js.map