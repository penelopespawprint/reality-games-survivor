 ...
 */
export function initializeAlerting(options) {
    config = {
 ...
 */
export async function alertJobFailure(execution) {
 ...
 */
async function sendEmailAlert(execution, isCritical) {
    if (!config.adminEmail) {
        return; // Email alerts disabled
    }
    const subject = isCritical
        ? `[RG:S] CRITICAL: Job "${execution.jobName}" Failed`
        : `[RG:S] Job Failure: ${execution.jobName}`;
    const html = `
    ...
    ${isCritical ? `
    <h3>⚠️ Critical Job - Immediate Action Required</h3>
    ...
    <p><em>Generated by Reality Games: Survivor Job Monitoring</em></p>
  `;
    const text = `
...
${isCritical ? '⚠️ CRITICAL JOB - Immediate action required!\n' : ''}
 ...
 */
async function sendSMSAlert(execution) {
    if (!config.adminPhone) {
        return; // SMS alerts disabled
    }
    const message = `[RG:S] CRITICAL: Job "${execution.jobName}" failed at ${execution.startTime.toLocaleTimeString()}. Error: ${(execution.error || 'Unknown').substring(0, 100)}... Check email for details.`;
    try {
...
export async function sendTestAlert() {
    ...
    if (config.adminPhone) {
        try {
            const result = await sendSMS({
                to: config.adminPhone,
                text: '[RG:S] Test alert: Job monitoring SMS is configured correctly.',
...
//# sourceMappingURL=jobAlerting.js.map