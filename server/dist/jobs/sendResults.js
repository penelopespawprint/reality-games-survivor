...
export async function sendEpisodeResults() {
    ...
    for (const league of leagues) {
        ...
        for (const member of members) {
            ...
            const castawayName = pick?.castaways?.name || 'No pick';
    ...
    for (const [userId, data] of userResults) {
        try {
            ...
            await EmailService.logNotification(userId, 'email', `Episode ${episode.number} Results`, `You earned ${data.pointsEarned} points.`);
            sent++;
        }
        catch (err) {
 ...
 */
export async function sendEliminationAlerts(episodeId) {
    ...
    for (const league of leagues) {
        ...
        for (const roster of affectedRosters) {
            ...
            try {
                ...
                await EmailService.logNotification(roster.user_id, 'email', `${castawayName} has been eliminated`, `You'll continue playing with your remaining castaway.`);
                sent++;
            }
            catch (err) {
...
//# sourceMappingURL=sendResults.js.map