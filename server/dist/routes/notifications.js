...
router.post('/send-reminders', requireAdmin, async (req, res) => {
    try {
        ...
        if (type === 'pick') {
            ...
            for (const league of leagues || []) {
                const deadline = new Date(league.seasons?.draft_deadline);
        ...
        if (notifications.length > 0) {
            const { error } = await supabaseAdmin.from('notifications').insert(notifications.map((n) => ({
                user_id: n.user_id,
                type: n.type,
                subject: n.subject,
                body: n.body,
    ...
    try {
        ...
        `)
            ...
            for (const member of members || []) {
                ...
                const castawayName = pick?.castaways?.name || 'Unknown';
                notifications.push({
                    user_id: member.user_id,
                    type: 'email',
        ...
        if (notifications.length > 0) {
            ...
            (async () => {
                ...
                const userLeagueResults = new Map();
                for (const notif of notifications) {
                    try {
                        const { data: user } = await supabaseAdmin
                            .from('users')
