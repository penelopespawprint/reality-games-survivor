...
import { validate, createLeagueSchema, joinLeagueSchema, updateLeagueSettingsSchema, } from '../../lib/validation.js';
...
router.post('/', authenticate, validate(createLeagueSchema), async (req, res) => {
    try {
...
router.post('/:id/join', authenticate, joinLimiter, validate(joinLeagueSchema), async (req, res) => {
    try {
        const userId = req.user.id;
...
router.patch('/:id/settings', authenticate, validate(updateLeagueSettingsSchema), async (req, res) => {
    try {
        const leagueId = req.params.id;
        const userId = req.user.id;
        const { name, password, donation_amount, payout_method, is_public, donation_notes, } = req.body;
        ...
        if (donation_notes !== undefined)
            updates.donation_notes = donation_notes;
