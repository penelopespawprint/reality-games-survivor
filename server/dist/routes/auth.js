...
import { normalizePhone, generateVerificationCode, sendVerificationSMS, } from '../config/twilio.js';
import { phoneLimiter, authLimiter } from '../config/rateLimit.js';
import { validate, updatePhoneSchema, verifyPhoneSchema, notificationPrefsSchema, } from '../lib/validation.js';
const router = Router();
...
router.patch('/me/phone', authenticate, phoneLimiter, validate(updatePhoneSchema), async (req, res) => {
    try {
...
router.post('/me/verify-phone', authenticate, authLimiter, validate(verifyPhoneSchema), async (req, res) => {
    try {
        const { code } = req.body;
        const userId = req.user.id;
...
router.post('/me/resend-code', authenticate, async (req, res) => {
    try {
        const userId = req.user.id;
        // Get user's phone
...
router.patch('/me/notifications', authenticate, validate(notificationPrefsSchema), async (req, res) => {
    try {
        ...
        if (sms !== undefined)
            updates.notification_sms = sms;
        if (push !== undefined)
            updates.notification_push = push;
