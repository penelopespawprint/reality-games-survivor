...
export async function getRevenueStats() {
    ...
    const lastMonthPayments = completedPayments.filter(p => p.created_at >= lastMonthStart && p.created_at < lastMonthEnd);
    ...
    const revenueByLeagueMap = new Map();
...
// ============================================================================
export async function getLeagueAnalyticsStats() {
...
// Game Progress Stats
// ============================================================================
export async function getGameProgressStats() {
    const now = DateTime.now().setZone('America/Los_Angeles');
    const weekStart = now.startOf('week').toISO();
    // Get active season
    ...
    `)
        ...
        .limit(1)
        .single();
    const topScorer = topMember && topMember.users
        ? {
            name: topMember.users.display_name,
...
export async function getSystemMetrics() {
    ...
    const dbStatus = latencyMs < 500 ? 'healthy' : latencyMs < 1000 ? 'degraded' : 'unhealthy';
    // Job stats
...
export async function getComprehensiveStats() {
    const [revenue, userEngagement, leagueAnalytics, communication, gameProgress, systemMetrics,] = await Promise.all([
        getRevenueStats(),
