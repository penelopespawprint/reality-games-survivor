...
export async function getDashboardStats() {
    ...
    try {
        ...
        const growthRate = newLastWeek.count && newLastWeek.count > 0
            ...
            : undefined;
        // League stats
        const [totalLeagues, leaguesWithActivity, globalLeague, allLeaguesWithMembers] = await Promise.all([
        ...
        const totalMembers = allLeaguesWithMembers.data?.reduce((sum, league) => {
            return sum + (league.league_members?.[0]?.count || 0);
        }, 0) || 0;
        const averageSize = totalLeagues.count && totalLeagues.count > 0 ? totalMembers / totalLeagues.count : 0;
        ...
        if (season) {
            const [picksThisWeek, totalPlayersThisWeek, castawaysRemaining, castawaysEliminated, episodesScored, totalEpisodes,] = await Promise.all([
                supabaseAdmin
                    .from('weekly_picks')
                    .select('id', { count: 'exact', head: true })
                    .gte('created_at', weekStart),
            ...
            const completionRate = totalPlayersThisWeek.count && totalPlayersThisWeek.count > 0
        ...
        const jobFailuresLast24h = jobHistory.filter((exec) => !exec.success && exec.startTime >= last24h).length;
        // Email queue stats
...
export async function getSystemHealth() {
    ...
    try {
        ...
        const last24h = now.minus({ hours: 24 }).toJSDate();
        const jobHistory = getJobHistory(100);
        const recentFailures = jobHistory.filter((exec) => !exec.success && exec.startTime >= last24h);
